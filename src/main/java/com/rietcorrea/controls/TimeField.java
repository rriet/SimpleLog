...
public class TimeField extends TextField {
    ...
    public TimeField() {
        ...
        this.addEventFilter(KeyEvent.KEY_TYPED, (KeyEvent inputevent) -> {
            int c = TimeField.this.getCaretPosition();
            // Only takes numbers and less than 5 characteres
            if (!"1234567890".contains(inputevent.getCharacter().toLowerCase()) || c > 8) {
                inputevent.consume();
            }
        });
        ...
        this.addEventFilter(KeyEvent.KEY_RELEASED, (KeyEvent inputevent) -> {
            ...
            int c = TimeField.this.getCaretPosition();
            if ((c == 5) && (inputevent.getCode() != KeyCode.LEFT && inputevent.getCode() != KeyCode.BACK_SPACE)) {
                TimeField.this.forward();
                inputevent.consume();
            }
        });
    }

    private TimeField(String time) {
        super(time);
        timePattern = Pattern.compile("([0-9]{5}):[0-5][0-9]");
        if (!validate(time)) {
    ...
    public boolean deleteNextChar() {
        ...
        if (selection.getLength() > 0) {
            ...
            if (caret != 5 && caret < 8) { // not preceeding a colon
    ...
    public boolean deletePreviousChar() {
        ...
        if (selection.getLength() > 0) {
            ...
            int caret = this.getCaretPosition();
            if (caret != 6 && caret != 0) { // not following a colon
                String currentText = this.getText();
    ...
    private class TimeIntegerConverter extends NumberStringConverter {
        ...
        public String toString(Number inputMinutes) {
            ...
            return String.format("%05d", hours) + ":" + String.format("%02d", minutes);
        }
    }
}
